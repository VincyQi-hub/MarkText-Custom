...
class ExportHtml {
  ...
  async renderHtml (toc) {
    ...
    // 自动分栏：将 float-left 图片与其后续内容（到下一标题/图片为止）组织为左右分栏
    this.autoWrapColumns(exportContainer)
  ...
  /**
   * 自动分栏：将 float-left 图片与其后续内容（到下一标题/图片/容器结束）
   * 组织为左右 flex 分栏（图片左侧固定，右侧内容填满）。
   * 仅在导出 HTML/PDF 时执行，编辑器内保持 float 环绕。
   */
  autoWrapColumns (container) {
    const blocks = Array.from(container.children)
    const results = []
    let i = 0
    while (i < blocks.length) {
      const block = blocks[i]
      const img = block.querySelector('img[data-align="float-left"]')
      if (img) {
        // 收集后续兄弟块（到下一标题或另一张浮动图片为止）
        const followBlocks = []
        let j = i + 1
        while (j < blocks.length) {
          const next = blocks[j]
          if (/^H[1-6]$/.test(next.tagName)) break
          if (next.querySelector('img[data-align]')) break
          followBlocks.push(next)
          j++
        }
        if (followBlocks.length > 0) {
          const col1 = document.createElement('div')
          col1.className = 'ag-columns-left'
          col1.appendChild(block)
          const col2 = document.createElement('div')
          col2.className = 'ag-columns-right'
          followBlocks.forEach(node => col2.appendChild(node))
          const cols = document.createElement('div')
          cols.className = 'ag-columns'
    ...
    results.forEach(node => container.appendChild(node))
  }
...
<head>
  ...
  <style>
    ...
    /* 表格环绕：用 .ag-table-wrap 容器建立 BFC（环绕浮动图片），表格本身保持 display:table */
    .markdown-body .ag-table-wrap {
      display: flow-root;
    }
    .markdown-body .ag-table-wrap[data-table-align="left"] {
      margin-left: 0;
      margin-right: auto;
    }
    .markdown-body .ag-table-wrap[data-table-align="center"] {
      margin-left: auto;
      margin-right: auto;
    }
    .markdown-body .ag-table-wrap[data-table-align="right"] {
      margin-left: auto;
      margin-right: 0;
    }
    .markdown-body .ag-table-wrap[data-table-align="full"] {
      width: 100%;
    }
    /* 左右分栏容器（flex）：第一个子块（图片）固定宽度，其余内容填充右侧 */
    .markdown-body .ag-columns {
      display: flex;
      align-items: flex-start;
      gap: 16px;
    }
    .markdown-body .ag-columns > *:first-child {
      flex: 0 0 auto;
    }
    .markdown-body .ag-columns > *:not(:first-child) {
      flex: 1 1 0;
      min-width: 0;
    }
    ...
    .markdown-body img[data-align="float-left"] {
      float: left;
      margin: 0 16px 8px 0;
    }
    .markdown-body img[data-align="float-right"] {
      float: right;
      margin: 0 0 8px 16px;
    }
    /* 建立 BFC：引用块/代码块/列表/标题等块级元素在浮动图片旁环绕显示在图片右侧；
       段落 p 不加，保持文字环绕图片的效果；表格通过 .ag-table-wrap 容器建立 BFC。 */
