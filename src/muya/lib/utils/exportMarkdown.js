...
class ExportMarkdown {
      ...
      switch (block.type) {
        ...
        case 'figure': {
          ...
          switch (block.functionType) {
            ...
            case 'columns': {
              result.push(this.normalizeColumns(block, indent))
              break
            }
            case 'mermaid':
            case 'flowchart':
  ...
  normalizeTable (table, indent) {
    ...
    const tableMarkdown = result.join('\n') + '\n'
    if (table.tableAlign) {
      return tableMarkdown + `${indent}{: align=${table.tableAlign} }\n`
    }
  ...
  normalizeColumns (block, indent) {
    const result = []
    result.push(`${indent}{: .columns }`)
    result.push(this.translateBlocks2Markdown(block.children, indent))
    result.push(`${indent}{: /columns }`)
    return result.join('\n') + '\n'
  }
