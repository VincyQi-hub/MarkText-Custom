...
class ImageToolbar extends BaseFloat {
  ...
  render () {
    ...
    const children = icons.map(i => {
      ...
      const isActive = i.type === dataAlign ||
        (i.type === 'wrap' && (dataAlign === 'float-left' || dataAlign === 'float-right'))
      if (isActive) {
        itemSelector += '.active'
      }
  ...
  selectItem (event, item) {
    ...
    switch (item.type) {
      ...
      case 'center':
      case 'right': {
        const align = item.type === 'wrap' ? 'float-left' : item.type
        this.muya.contentState.updateImage(this.imageInfo, 'data-align', align)
        return this.hide()
      }
    }
  }
