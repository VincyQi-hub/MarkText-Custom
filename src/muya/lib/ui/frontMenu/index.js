...
class FrontMenu extends BaseFloat {
  ...
  render () {
    ...
    const isTableBlock = type === 'figure' && functionType === 'table'
    const children = menu.map(({ icon, label, text, shortCut }) => {
      // 表格对齐按钮仅在表格块时显示
      if (/^table-align-/.test(label) && !isTableBlock) {
        return null
      }
    ...
    const vnode = h('ul', children.filter(Boolean))
  ...
  selectItem (event, { label }) {
    ...
    switch (label) {
      ...
      case 'table-align-full': {
        contentState.setTableAlign(this.outmostBlock, label.split('-').pop())
        break
      }
      default:
