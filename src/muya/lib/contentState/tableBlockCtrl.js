...
const tableBlockCtrl = ContentState => {
  ...
  /**
   * 设置表格整体对齐（不依赖光标位置，从表格 figure 块定位 table）。
   * @param {Object} figureBlock 表格所在的 figure 块
   * @param {string} align left | center | right | full
   */
  ContentState.prototype.setTableAlign = function (figureBlock, align) {
    const table = figureBlock && figureBlock.children ? figureBlock.children[0] : null
    if (table && table.type === 'table') {
      table.tableAlign = align
      // 用 singleRender 强制渲染表格块，避免 partialRender 渲染范围不含表格导致不立即生效
      this.singleRender(figureBlock, true)
      this.muya.dispatchChange()
    }
  }
  ...
  ContentState.prototype.editTable = function ({ location, action, target }, cellContentKey) {
    ...
    // 表格整体对齐：设置 table block 的 tableAlign（left/center/right）
    if (target === 'table' && action === 'align') {
      const table = this.closest(block, 'table')
      if (table) {
        table.tableAlign = location
        this.partialRender()
        this.muya.dispatchChange()
      }
      return
