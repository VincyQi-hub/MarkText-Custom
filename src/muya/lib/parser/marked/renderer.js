...
Renderer.prototype.table = function (header, body, tableAlign) {
  if (body) body = '<tbody>' + body + '</tbody>'

  const alignAttr = tableAlign ? ` data-table-align="${tableAlign}"` : ''

  // 用 .ag-table-wrap 包裹：容器建立 BFC（环绕浮动图片），表格保持 display:table 布局
  return '<div class="ag-table-wrap"><table' + alignAttr + '>\n' +
    '<thead>\n' +
    header +
    '</thead>\n' +
    body +
