  ...
  while (src) {
    ...
    if (cap) {
      ...
      if (item.header.length === item.align.length) {
        ...
        // 表格整体对齐自定义语法：表格后一行 `{: align=left|center|right }`
        const lastRow = item.cells[item.cells.length - 1]
        if (Array.isArray(lastRow) && /^\{: align=(left|center|right|full) ?\}$/.test((lastRow[0] || '').trim()) && lastRow.slice(1).every(c => !c.trim())) {
          const alignMatch = /^\{: align=(left|center|right|full) ?\}$/.exec(lastRow[0].trim())
          if (alignMatch) {
            item.tableAlign = alignMatch[1]
            item.cells.pop()
          }
        } else if (Array.isArray(lastRow) && /^\{: /.test((lastRow[0] || '').trim()) && lastRow.slice(1).every(c => !c.trim())) {
          // 表格后紧跟其他自定义标记行（如分栏结束 {: /columns }）：弹出并回填，让主循环解析
          item.cells.pop()
          src = lastRow[0].trim() + '\n' + src
        }
    ...
    // columns container（自定义分栏语法 {: .columns } ... {: /columns }）
    cap = /^\{: \.columns ?\}\n?/.exec(src)
    if (cap) {
      src = src.substring(cap[0].length)
      this.tokens.push({ type: 'columns_start' })
      continue
    }
    cap = /^\{: \/columns ?\}\n?/.exec(src)
    if (cap) {
      src = src.substring(cap[0].length)
      this.tokens.push({ type: 'columns_end' })
      continue
    }
