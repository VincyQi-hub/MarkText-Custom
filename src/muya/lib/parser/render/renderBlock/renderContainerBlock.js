  ...
  if (/^(?:th|td)$/.test(type)) {
    ...
    if (functionType) {
      ...
      if (functionType === 'table') {
        // 表格整体对齐属性放在 figure 层（配合 CSS width:fit-content + margin，
        // 使 front icon 按钮跟随表格位置）
        const tableBlock = block.children[0]
        if (tableBlock && tableBlock.tableAlign) {
          Object.assign(data.attrs, { 'data-table-align': tableBlock.tableAlign })
        }
      }
      if (functionType === 'columns') {
        // 左右分栏容器（flex 布局：第一个子块为图片，其余内容填充右侧）
        selector += '.ag-columns'
      }
